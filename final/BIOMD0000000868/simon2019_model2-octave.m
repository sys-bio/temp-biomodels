...
%
% Model name = Simon2019 - NIK-dependent p100 processing into p52, Mass Action, SBML 2v4
%
% isDescribedBy http://identifiers.org/pubmed/31048505
% is http://identifiers.org/biomodels.db/MODEL1903280002
% is http://identifiers.org/biomodels.db/BIOMD0000000868
%
...
% Reaction: id = p100_processing, name = p100 processing	% Local Parameter:   id =  k1, name = k1
	...
	reaction_p100_processing=compartment_compartment*reaction_p100_processing_k1*x(4);
...
% Reaction: id = p100_NIK_binding, name = p100 NIK binding	% Local Parameter:   id =  k1, name = k1
	...
	reaction_p100_NIK_binding=compartment_compartment*(reaction_p100_NIK_binding_k1*x(1)*x(3)-reaction_p100_NIK_binding_k2*x(4));
...
% Species:   id = p100, name = p100, affected by kineticLaw
	xdot(1) = (1/(compartment_compartment))*(( 1.0 * reaction_p100_mRNA_Expressions) + (-1.0 * reaction_p100_degradation) + (-1.0 * reaction_p100_NIK_binding));
...
% Species:   id = NIK, name = NIK, affected by kineticLaw
	xdot(3) = (1/(compartment_compartment))*(( 1.0 * reaction_p100_processing) + (-1.0 * reaction_p100_NIK_binding));
	
% Species:   id = p100_NIK, name = p100:NIK, affected by kineticLaw
	xdot(4) = (1/(compartment_compartment))*((-1.0 * reaction_p100_processing) + ( 1.0 * reaction_p100_NIK_binding));
end
