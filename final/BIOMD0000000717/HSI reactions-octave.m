...
% Reaction: id = reaction_3, name = Infection of susceptible bird population
	reaction_reaction_3=compartment_compartment*Rate_for_reaction_3(global_par_beta_b, x(1), x(2), global_par_H_b);
...
% Reaction: id = reaction_8, name = Infection of susceptible human population
	reaction_reaction_8=compartment_compartment*Rate_for_reaction_8__1(global_par_beta_bh, x(3), x(2), global_par_H_bh);

% Reaction: id = reaction_9, name = Disease related death of infected human population
	reaction_reaction_9=compartment_compartment*global_par_d*x(4);

% Reaction: id = reaction_10, name = Natural death of infected human ppopulation
	reaction_reaction_10=compartment_compartment*global_par_mu_h*x(4);
...
% Species:   id = S_h, name = Susceptible human (S_h), affected by kineticLaw
	xdot(3) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_6) + (-1.0 * reaction_reaction_7) + (-1.0 * reaction_reaction_8));
...
function z=Rate_for_reaction_3(beta_b,S_b,I_b,H_b), z=(beta_b*S_b*I_b/(H_b+I_b));end

function z=Rate_for_reaction_8__1(beta_bh,S_h,I_b,H_bh), z=(beta_bh*S_h*I_b/(H_bh+I_b));end
