...
% is http://identifiers.org/biomodels.db/MODEL1907100001
% is http://identifiers.org/biomodels.db/BIOMD0000000746
%
...
% Reaction: id = cancer_killing, name = cancer killing
	reaction_cancer_killing=compartment_bladder_cancer_tme*cancer_killing_0(global_par_alpha1, x(2), x(1), x(4), global_par_k);

% Reaction: id = effector_activation_by_cancer, name = effector activation by cancer
	reaction_effector_activation_by_cancer=compartment_bladder_cancer_tme*effector_activation(global_par_a1, x(1), x(2), x(4), global_par_k);

% Reaction: id = effector_activation_by_BCG, name = effector activation by BCG
	reaction_effector_activation_by_BCG=compartment_bladder_cancer_tme*effector_activation(global_par_a2, x(3), x(2), x(4), global_par_k);
...
% Reaction: id = checkpoint_decay, name = checkpoint decay
	reaction_checkpoint_decay=compartment_bladder_cancer_tme*global_par_u3*x(4);
...
% Species:   id = P, name = P, affected by kineticLaw
	xdot(4) = (1/(compartment_bladder_cancer_tme))*(( 1.0 * reaction_checkpoint_source) + (-1.0 * reaction_checkpoint_decay));
end

function z=effector_activation(a,modifier,E,P,k), z=(a*modifier*E/(P+k));end
...
function z=cancer_killing_0(alpha1,E,C,P,k), z=(alpha1*E*C/(P+k));end
