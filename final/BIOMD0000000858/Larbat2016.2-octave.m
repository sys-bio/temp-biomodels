...
% assignmentRule: variable = Light_Dark_Tracker
	global_par_Light_Dark_Tracker=piecewise(time-24*ceil(time/24),  xor (time < 0, 24 < 0), time-24*floor(time/24))-12;
...
% Reaction: id = Sucrose_Formation_Starch, name = Sucrose_Formation_Starch
	reaction_Sucrose_Formation_Starch=compartment_compartment*Function_for_Sucrose_Formation_Starch(global_par_k37, x(5), x(8));
...
% Reaction: id = Starch_Formation, name = Starch_Formation
	reaction_Starch_Formation=compartment_compartment*global_par_k10*x(7);
...
% Reaction: id = ECO2_Consumption, name = ECO2_Consumption
	reaction_ECO2_Consumption=compartment_compartment*Function_for_ECO2_Consumption(global_par_k31, x(7), const_species_ECO2, global_par_k32);

% Reaction: id = Estarch_Consumption, name = Estarch_Consumption
	reaction_Estarch_Consumption=compartment_compartment*Function_for_Estarch_Consumption(global_par_k35, x(1), x(8), global_par_k36);
...
% Species:   id = Estarch, name = Estarch, affected by kineticLaw
	xdot(8) = (1/(compartment_compartment))*(( 1.0 * reaction_Estarch_Formation) + (-1.0 * reaction_Estarch_Consumption));
end
