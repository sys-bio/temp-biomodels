...
%
% Model name = Reyes-Palomares2012 -  a combined model hepatic polyamine and sulfur aminoacid metabolism - version2
%
% is http://identifiers.org/biomodels.db/MODEL1305060001
% is http://identifiers.org/biomodels.db/BIOMD0000000450
...
% Parameter:   id =  parameter_19, name = Vm_MATII
	global_par_parameter_19=220.0;
...
% Reaction: id = reaction_13, name = V_MATII	% Local Parameter:   id =  Ki, name = Ki
	reaction_reaction_13_Ki=50.0;
	% Local Parameter:   id =  Km, name = Km
	reaction_reaction_13_Km=4.0;

	reaction_reaction_13=compartment_cytosol*function_4_V_MATII(reaction_reaction_13_Ki, reaction_reaction_13_Km, x(26), global_par_parameter_19, x(25));
...
% Species:   id = H2O2, name = H2O2, constant	const_species_H2O2=0.015;
...
% Species:   id = HCHO, name = Formaldehyde, constant	const_species_HCHO=500.0;
	...
	xdot=zeros(48,1);
	% rateRule: variable = parameter_1
	xdot(45) = 60*65.06/x(25)*global_par_parameter_7*1/(1+global_par_parameter_5*(x(31)+x(30)))-global_par_parameter_6*global_par_parameter_4*global_par_parameter_1;
	% rateRule: variable = parameter_2
	...
	% rateRule: variable = parameter_3
	xdot(47) = 60*65.06/x(25)*global_par_parameter_11*1/(1+global_par_parameter_5*(x(31)+x(30)))-global_par_parameter_10*global_par_parameter_3;
	% rateRule: variable = parameter_4
...
% Species:   id = sam, name = SAM, affected by kineticLaw
	xdot(25) = (1/(compartment_cytosol))*(( 1.0 * reaction_V_MATI) + ( 1.0 * reaction_V_MATIII) + (-1.0 * reaction_V_GNMT) + (-1.0 * reaction_V_DNMT) + (-1.0 * reaction_reaction_2) + ( 1.0 * reaction_reaction_13));
	
% Species:   id = met, name = c_Methionine, affected by kineticLaw
	xdot(26) = (1/(compartment_cytosol))*(( 1.0 * reaction_V_b_MET_c) + ( 1.0 * reaction_V_MS) + ( 1.0 * reaction_V_BHMT) + (-1.0 * reaction_V_MATI) + (-1.0 * reaction_V_MATIII) + (-1.0 * reaction_reaction_13));
...
function z=function_4_V_MATII(Ki,Km,met,parameter_19,sam), z=(parameter_19/(1+(Km*(1+sam/Ki)/met)^0.76));end
