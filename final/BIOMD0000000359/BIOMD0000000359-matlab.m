...
%
% Model name = Panteleev2002_TFPImechanism_schmema3
%
% is http://identifiers.org/biomodels.db/MODEL1108260008
% is http://identifiers.org/biomodels.db/BIOMD0000000359
% isDescribedBy http://identifiers.org/pubmed/11985578
% isDerivedFrom http://identifiers.org/pubmed/9468488
%
...
% Reaction: id = reaction_5, name = reaction_5	% Local Parameter:   id =  k1, name = k1
	...
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_5_k2=0.0;

	reaction_reaction_5=compartment_compartment*(reaction_reaction_5_k1*x(1)*x(7)-reaction_reaction_5_k2*x(8));

% Reaction: id = reaction_6, name = reaction_6	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_6_k1=6.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_6_k2=0.02;

	reaction_reaction_6=compartment_compartment*(reaction_reaction_6_k1*x(4)*x(6)-reaction_reaction_6_k2*x(9));

% Reaction: id = reaction_9, name = reaction_8	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_9_k1=0.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_9_k2=0.0;

	reaction_reaction_9=compartment_compartment*(reaction_reaction_9_k1*x(9)-reaction_reaction_9_k2*x(8));

% Reaction: id = reaction_8, name = reaction_9	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_8_k1=20.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_8_k2=0.0;

	reaction_reaction_8=compartment_compartment*(reaction_reaction_8_k1*x(3)*x(7)-reaction_reaction_8_k2*x(2)*x(9));
...
% Species:   id = X, name = X, affected by kineticLaw
	xdot(2) = (1/(compartment_compartment))*((-1.0 * reaction_reaction_1) + ( 1.0 * reaction_reaction_8));
	
% Species:   id = VIIa_TF_X, name = VIIa_TF_X, affected by kineticLaw
	xdot(3) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_1) + (-1.0 * reaction_reaction_2) + (-1.0 * reaction_reaction_8));
	
% Species:   id = VIIa_TF_Xa, name = VIIa_TF_Xa, affected by kineticLaw
	xdot(4) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_2) + (-1.0 * reaction_reaction_3) + (-1.0 * reaction_reaction_6));
...
% Species:   id = TFPI, name = TFPI, affected by kineticLaw
	xdot(6) = (1/(compartment_compartment))*((-1.0 * reaction_reaction_4) + (-1.0 * reaction_reaction_6));
	
% Species:   id = Xa_TFPI, name = Xa_TFPI, affected by kineticLaw
	xdot(7) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_4) + (-1.0 * reaction_reaction_5) + (-1.0 * reaction_reaction_8));
	
% Species:   id = Xa_TFPI_VIIa_TF, name = Xa_TFPI_VIIa_TF, affected by kineticLaw
	xdot(8) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_5) + ( 1.0 * reaction_reaction_9));
	
% Species:   id = VIIa_TF_Xa_TFPI, name = VIIa_TF_Xa_TFPI, affected by kineticLaw
	xdot(9) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_6) + (-1.0 * reaction_reaction_9) + ( 1.0 * reaction_reaction_8));
end
