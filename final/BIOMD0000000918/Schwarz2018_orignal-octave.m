...
% Reaction: id = For_Myc, name = For Myc
	reaction_For_Myc=compartment_Cell*Rate_Law_for_For_M__1__1(global_par_KS, x(1), global_par_dM, global_par_kM, x(8));
...
% Reaction: id = for_Phosphorylated_Rb, name = for Phosphorylated Rb
	reaction_for_Phosphorylated_Rb=compartment_Cell*Rate_Law_for_for_RP__1__1(x(3), x(4), global_par_KCD, global_par_KCE, x(6), x(5), x(7), global_par_dRP, global_par_kP1, global_par_kP2);

% Reaction: id = for_Rb_E2F_complex, name = for Rb-E2F complex
	reaction_for_Rb_E2F_complex=compartment_Cell*Rate_Law_for_for_RE__1__1(x(3), x(4), global_par_KCD, global_par_KCE, x(7), global_par_dRE, global_par_kP1, global_par_kP2, global_par_kRE);

%Event: id=Serum_Pulse
	event_Serum_Pulse=time > 5;
...
function z=Rate_Law_for_For_M__1__1(KS,Myc,dM,kM,serum), z=(kM*Myc/(KS+serum)-dM*Myc);end
...
function z=Rate_Law_for_for_RP__1__1(CycD,CycE,KCD,KCE,Phosphorylated_Rb,Rb,Rb_E2F_complex,dRP,kP1,kP2), z=(kP1*CycD*Rb/(KCD+Rb)+kP2*CycE*Rb/(KCE+Rb_E2F_complex)+kP1*CycD*Rb_E2F_complex/(KCD+Rb_E2F_complex)+kP2*CycE*Rb_E2F_complex/(KCE+Rb_E2F_complex)-dRP*Phosphorylated_Rb);end

function z=Rate_Law_for_for_RE__1__1(CycD,CycE,KCD,KCE,Rb_E2F_complex,dRE,kP1,kP2,kRE), z=(kRE-kP1*CycD*Rb_E2F_complex/(KCD+Rb_E2F_complex)+kP2*CycE*Rb_E2F_complex/(KCE+Rb_E2F_complex)-dRE*Rb_E2F_complex);end

function z=Rate_Law_for_For_CD_0(kCD,M,KM,kCDS,S,KS,dCD,CD), z=(kCD*M/(KM+M)+kCDS*S/(KS+S)-dCD*CD);end

function z=Rate_Law_for_for_CE_0(kCE,E,KE,dCE,CE), z=(kCE*E/(KE+E)-dCE*CE);end

function z=Rate_Law_for_For_R_0(kR,kDP,RP,KRP,kRE,R,E,kP1,CD,KCD,kP2,CE,KCE,dR), z=(kR+kDP*RP/(KRP+RP)-kRE*R*E-kP1*CD*R/(KCD+R)-kP2*CE*R/(KCE+R)-dR*R);end

function z=Rate_Law_for_For_M__1_0(kM,M,KS,S,dM), z=(kM*M/(KS+S)-dM*M);end

function z=Rate_Law_for_For_E__1_0(kE,kpfb,M,KM,E,KE,kb,kp1,CD,RE,KCD,kp2,CE,KCE,dE,kRE,R), z=(kE*(kpfb+M/(KM+M))*E/(KE+E)+kb*M/(KM+M)+kp1*CD*RE/(KCD+RE)+kp2*CE*RE/(KCE+RE)-dE*E-kRE*R*E);end

function z=Rate_Law_for_for_RE__1_0(kRE,kP1,CD,KCD,kP2,CE,RE,KCE,dRE), z=(kRE-kP1*CD*RE/(KCD+RE)+kP2*CE*RE/(KCE+RE)-dRE*RE);end

function z=Rate_Law_for_for_RP__1_0(kP1,CD,R,KCD,kP2,CE,KCE,RE,dRP,RP), z=(kP1*CD*R/(KCD+R)+kP2*CE*R/(KCE+RE)+kP1*CD*RE/(KCD+RE)+kP2*CE*RE/(KCE+RE)-dRP*RP);end
