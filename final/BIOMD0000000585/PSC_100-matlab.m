...
%
% Model name = js_psc100_rationeu_1e8
%
%
...
%Initial conditions vector
	...
	x0(21) = 0.9091525;
	x0(22) = 0.0;
	x0(23) = 0.117925561795;
	x0(24) = 1.00727491509661;
	x0(25) = 0;
...
% Parameter:   id =  scale_Stat1Pex, name = WB_STAT1D
	global_par_scale_Stat1Pex=12.1866;
% Parameter:   id =  scale_Socs1ex, name = PCR_SOCS1
	global_par_scale_Socs1ex=0.566011;
...
% assignmentRule: variable = Socs1ex
	x(23)=x(20)*global_par_scale_Socs1ex;
% assignmentRule: variable = RSNCex
	x(24)=(x(19)+x(8))/(x(18)+x(7));
% assignmentRule: variable = RSPNCex
	x(25)=x(8)/x(7);
...
x(20) = x(45);
	...
	xdot=zeros(45,1);
	% rateRule: variable = Ifng
	xdot(26) = -global_par_k0*x(1)-global_par_k1*x(1)*x(17)+global_par_k2*x(2);
	% rateRule: variable = II
	xdot(27) = global_par_k1*x(1)*x(17)-global_par_k2*x(2);
	% rateRule: variable = d1
	xdot(28) = 4*(x(2)-x(3))/global_par_taud;
	% rateRule: variable = d2
	xdot(29) = 4*(x(3)-x(4))/global_par_taud;
	% rateRule: variable = d3
	xdot(30) = 4*(x(4)-x(5))/global_par_taud;
	% rateRule: variable = d4
	xdot(31) = 4*(x(5)-x(6))/global_par_taud;
	% rateRule: variable = Stat1Pd
	xdot(32) = global_par_k4*x(2)*x(18)/(1+global_par_k14*x(16))-global_par_k6*x(7);
	% rateRule: variable = Stat1Pdn
	xdot(33) = 3*global_par_k6*x(7)-global_par_k5*x(8);
	% rateRule: variable = i1
	xdot(34) = 4*(x(8)-x(9))/global_par_tau;
	% rateRule: variable = i2
	xdot(35) = 4*(x(9)-x(10))/global_par_tau;
	% rateRule: variable = i3
	xdot(36) = 4*(x(10)-x(11))/global_par_tau;
	% rateRule: variable = i4
	xdot(37) = 4*(x(11)-x(12))/global_par_tau;
	% rateRule: variable = j1
	xdot(38) = 4*(x(20)-x(13))/global_par_tauj;
	% rateRule: variable = j2
	xdot(39) = 4*(x(13)-x(14))/global_par_tauj;
	% rateRule: variable = j3
	xdot(40) = 4*(x(14)-x(15))/global_par_tauj;
	% rateRule: variable = j4
	xdot(41) = 4*(x(15)-x(16))/global_par_tauj;
	% rateRule: variable = Ir
	xdot(42) = -global_par_k1*x(1)*x(17)+global_par_k2*x(2);
	% rateRule: variable = Stat1U
	xdot(43) = global_par_k3*x(6)-global_par_k11*x(18)+global_par_k12*x(19)/3-global_par_k4*x(2)*x(18)/(1+global_par_k14*x(16));
	% rateRule: variable = Stat1Un
	xdot(44) = 3*global_par_k11*x(18)-global_par_k12*x(19)+global_par_k5*x(8);
	% rateRule: variable = mRNA
	xdot(45) = global_par_k13+global_par_k9*x(12)-global_par_k10*x(20);
...
% Species:   id = RSNCex, name = RSNC (observed), involved in a rule 	xdot(24) = x(24);
	
% Species:   id = RSPNCex, name = RSPNC (observed), involved in a rule 	xdot(25) = x(25);
end
