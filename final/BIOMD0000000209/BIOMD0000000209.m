...
%
% Model name = Chickarmane2008 - Stem cell lineage determination
%
% is http://identifiers.org/biomodels.db/MODEL8390025091
% is http://identifiers.org/biomodels.db/BIOMD0000000209
...
% Parameter:   id =  gammag, name = gammag
	global_par_gammag=0.1;
% Parameter:   id =  gamman, name = gamman
	global_par_gamman=0.01;

% Reaction: id = R1, name = R1
	reaction_R1=(global_par_a0+global_par_a1*x(10)+global_par_a2*x(13)*x(14)+global_par_a3*x(13)*x(14)*x(15))/(1+global_par_b0*x(10)+global_par_b1*x(13)+global_par_b2*x(13)*x(14)+global_par_b3*x(13)*x(14)*x(15)+global_par_b4*x(17)*x(13)+global_par_b5*x(18));

% Reaction: id = R2, name = R2
	reaction_R2=global_par_gamma1*x(13);

% Reaction: id = R3, name = R3
	reaction_R3=(global_par_c0+global_par_c1*x(13)*x(14)+global_par_c2*x(13)*x(14)*x(15))/(1+global_par_d0*x(13)+global_par_d1*x(13)*x(14)+global_par_d2*x(13)*x(14)*x(15));

% Reaction: id = R4, name = R4
	reaction_R4=global_par_gamma2*x(14);

% Reaction: id = R5, name = R5
	reaction_R5=(global_par_e0+global_par_e1*x(13)*x(14)+global_par_e2*x(13)*x(14)*x(15)+global_par_e3*x(12))/(1+global_par_f0*x(13)+global_par_f1*x(13)*x(14)+global_par_f2*x(13)*x(14)*x(15)+global_par_f3*x(13)*x(16)+global_par_f4*x(12));

% Reaction: id = R6, name = R6
	reaction_R6=global_par_gamma3*x(15);

% Reaction: id = R7, name = R7
	reaction_R7=(global_par_g0+global_par_g1*x(17))/(1+global_par_h0*x(17)+global_par_h1*x(17)*x(13));

% Reaction: id = R8, name = R8
	reaction_R8=global_par_gamma4*x(17);

% Reaction: id = R9, name = R9
	reaction_R9=(global_par_i0+global_par_i1*x(17)+global_par_i2*x(16))/(1+global_par_j0*x(17)+global_par_j1*x(16));

% Reaction: id = R10, name = R10
	reaction_R10=global_par_gamma5*x(18);

% Reaction: id = R11, name = R11
	reaction_R11=(global_par_p0+global_par_p1*x(13)+global_par_p2*x(16))/(1+global_par_q0*x(13)+global_par_q1*x(16)+global_par_q2*x(15)+global_par_q3*x(11));

% Reaction: id = R12, name = R12
	reaction_R12=global_par_gammag*x(16);
...
% Species:   id = SG, name = SG
%WARNING speciesID: SG, constant= false  , boundaryCondition = true but is not involved in assignmentRule, rateRule or events !
	xdot(11) = 0.0;
	
% Species:   id = SN, name = SN
%WARNING speciesID: SN, constant= false  , boundaryCondition = true but is not involved in assignmentRule, rateRule or events !
	xdot(12) = 0.0;
	
% Species:   id = OCT4, name = OCT4, affected by kineticLaw
	xdot(13) = (1/(compartment_cell))*(( 1.0 * reaction_R1) + (-1.0 * reaction_R2));
	
% Species:   id = SOX2, name = SOX2, affected by kineticLaw
	xdot(14) = (1/(compartment_cell))*(( 1.0 * reaction_R3) + (-1.0 * reaction_R4));
	
% Species:   id = NANOG, name = NANOG, affected by kineticLaw
	xdot(15) = (1/(compartment_cell))*(( 1.0 * reaction_R5) + (-1.0 * reaction_R6));
	
% Species:   id = GATA6, name = GATA6, affected by kineticLaw
	xdot(16) = (1/(compartment_cell))*(( 1.0 * reaction_R11) + (-1.0 * reaction_R12));
	
% Species:   id = CDX2, name = CDX2, affected by kineticLaw
	xdot(17) = (1/(compartment_cell))*(( 1.0 * reaction_R7) + (-1.0 * reaction_R8));
	
% Species:   id = GCNF, name = GCNF, affected by kineticLaw
	xdot(18) = (1/(compartment_cell))*(( 1.0 * reaction_R9) + (-1.0 * reaction_R10));
