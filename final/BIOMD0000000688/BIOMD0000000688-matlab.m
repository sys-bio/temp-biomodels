...
% Reaction: id = NK_infection_induced_production, name = NK infection-induced production
	reaction_NK_infection_induced_production=compartment_COMpartment*function_4(global_par_ci, x(2), x(3), x(16));

% Reaction: id = NK_cell_death, name = NK cell death
	reaction_NK_cell_death=compartment_COMpartment*global_par_bi*x(16);

% Reaction: id = NK_induced_infected_cell_death_1, name = NK-induced infected cell death 1
	reaction_NK_induced_infected_cell_death_1=compartment_COMpartment*function(global_par_p_i, x(16), x(2));

% Reaction: id = NK_induced_infected_cell_death_2, name = NK-induced infected cell death 2
	reaction_NK_induced_infected_cell_death_2=compartment_COMpartment*function(global_par_p_i, x(16), x(3));
...
% Species:   id = y_0, name = y_0, affected by kineticLaw
	xdot(2) = (1/(compartment_COMpartment))*(( 1.0 * reaction_Infection_1) + (-1.0 * reaction_Infected_cell_death_1) + (-1.0 * reaction_Infected_cell_infection_progression_1) + ( 1.0 * reaction_Infected_cell_infection_progression_2) + (-1.0 * reaction_CTL_induced_infected_cell_death_1) + (-1.0 * reaction_NK_induced_infected_cell_death_1));
	
% Species:   id = y_1, name = y_1, affected by kineticLaw
	xdot(3) = (1/(compartment_COMpartment))*(( 1.0 * reaction_Infected_cell_infection_progression_1) + (-1.0 * reaction_Infected_cell_death_2) + (-1.0 * reaction_CTL_induced_infected_cell_death_2) + (-1.0 * reaction_NK_induced_infected_cell_death_2));
...
% Species:   id = z_i, name = z_i, affected by kineticLaw
	xdot(16) = (1/(compartment_COMpartment))*(( 1.0 * reaction_NK_cell_production) + ( 1.0 * reaction_NK_infection_induced_production) + (-1.0 * reaction_NK_cell_death));
end
...
function z=function_4(c_i,y0,y1,zi), z=(c_i*(y0+y1)*zi);end
