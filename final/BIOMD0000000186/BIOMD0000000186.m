...
% Reaction: id = R2
	reaction_R2=global_par_u*global_par_k2f*x(3)*compartment_Cytoplasm*x(4);
...
% Reaction: id = R7a
	reaction_R7a=global_par_u_prime*global_par_k7r*x(10)*compartment_Cytoplasm;
...
% Species:   id = MCC, name = MCC, affected by kineticLaw
	xdot(7) = (1/(compartment_Cytoplasm))*(( 1.0 * reaction_R4) + (-1.0 * reaction_R7) + ( 1.0 * reaction_R7a));
...
% Species:   id = APC, name = APC, affected by kineticLaw
	xdot(9) = (1/(compartment_Cytoplasm))*((-1.0 * reaction_R7) + ( 1.0 * reaction_R7a) + (-1.0 * reaction_R8));
	
% Species:   id = MCC_APC, name = MCC:APC, affected by kineticLaw
	xdot(10) = (1/(compartment_Cytoplasm))*(( 1.0 * reaction_R7) + (-1.0 * reaction_R7a));
	
% Species:   id = APC_Cdc20, name = APC:Cdc20, affected by kineticLaw
	xdot(11) = (1/(compartment_Cytoplasm))*(( 1.0 * reaction_R8));
end
