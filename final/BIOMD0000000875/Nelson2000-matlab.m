...
% Reaction: id = T_cell_loss, name = loss of uninfected T-cell
	reaction_T_cell_loss=compartment_plasma*global_par_delta1*x(1);

% Reaction: id = infection, name = T-cell infection
	reaction_infection=compartment_plasma*Function_for_T_cell_infection_1(x(1), x(3), global_par_k);

% Reaction: id = T_i_lysis, name = infected T-cell lysis
	reaction_T_i_lysis=compartment_plasma*global_par_delta*x(2);

% Reaction: id = T_lysis_infect, name = release of infectious virions on lysis
	reaction_T_lysis_infect=compartment_plasma*Function_for_release_of_infectious_virions_on_lysis_1(global_par_N, x(2), global_par_delta, global_par_np);

% Reaction: id = T_lysis_noninf, name = release of non infectious virions on lysis
	reaction_T_lysis_noninf=compartment_plasma*Function_for_release_of_non_infectious_virions_on_lysis(global_par_N, x(2), global_par_delta, global_par_np);

% Reaction: id = V_I_clearance, name = clearance of infectious virions
	reaction_V_I_clearance=compartment_plasma*Function_for_clearance_of_infectious_virions(x(3), global_par_c);

% Reaction: id = V_NI_clearance, name = non-infect. virion clearance
	reaction_V_NI_clearance=compartment_plasma*global_par_c*x(4);
...
function z=Function_for_T_cell_infection_1(T,V_I,k), z=(k*V_I*T);end

function z=Function_for_release_of_infectious_virions_on_lysis_1(N,T_i,delta,np), z=((1-np)*N*delta*T_i);end

function z=Function_for_clearance_of_infectious_virions(V_I,c), z=(c*V_I);end

function z=Function_for_release_of_non_infectious_virions_on_lysis(N,T_i,delta,np), z=(np*N*delta*T_i);end
