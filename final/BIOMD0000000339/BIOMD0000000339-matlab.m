...
%
% Model name = Wajima2009_BloodCoagulation_PTtest
%
% is http://identifiers.org/biomodels.db/MODEL1107010001
% is http://identifiers.org/biomodels.db/BIOMD0000000339
...
% Compartment: id = compartment_1, name = compartment_1
% Warning compartment compartment_1 has no rule and is not constant
...
global_par_Integral_Fibrin = x(57);
...
%Event: id=clottingTime_event
	event_clottingTime_event=(global_par_Integral_Fibrin*3600) > 1500;

	if(event_clottingTime_event) 
		global_par_clottingTime_s=time*3600;
	end

%Event: id=dilution_event
	event_dilution_event=time > 0;
...
% Species:   id = ATIII_Heparin, name = ATIII_Heparin, affected by kineticLaw
	xdot(40) = (1/(compartment_compartment_1))*((-1.0 * reaction_r44) + (-1.0 * reaction_r45) + (-1.0 * reaction_r46) + (-1.0 * reaction_eHeparin));
