...
%
% Model name = Ayati2010_BoneRemodelingDynamics_WithTumour
%
% is http://identifiers.org/biomodels.db/MODEL1112060001
% is http://identifiers.org/biomodels.db/BIOMD0000000402
...
x(4) = x(8);
	...
	% rateRule: variable = B
	xdot(6) = global_par_alpha2*x(1)^(global_par_g12/(1+global_par_r12*x(4)/global_par_LT))*x(2)^(global_par_g22-global_par_r22*x(4)/global_par_LT)-global_par_beta2*x(2);
	% rateRule: variable = z
	xdot(7) = global_par_k2*global_par_y2-global_par_k1*global_par_y1;
	% rateRule: variable = Tumour
	xdot(8) = global_par_gammaT*x(4)*log(global_par_LT/x(4));
