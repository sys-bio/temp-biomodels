...
%
% Model name = Gevertz2018 - Cancer Treatment with Oncolytic Viruses and Dendritic Cell injections original model
%
% is http://identifiers.org/biomodels.db/MODEL1909180001
% is http://identifiers.org/biomodels.db/BIOMD0000000816
%
...
% Parameter:   id =  C_T, name = C_T
	global_par_C_T=1.698362;
% Parameter:   id =  chi_A, name = chi_A
	global_par_chi_A=1.0;
% Parameter:   id =  chi_D, name = chi_D
	global_par_chi_D=4.675397;
% Parameter:   id =  delta_T, name = delta_T
	global_par_delta_T=0.35;
% Parameter:   id =  C_A, name = C_A
	global_par_C_A=5.17E-4;
% Parameter:   id =  delta_A, name = delta_A
	global_par_delta_A=0.35;
...
% Reaction: id = Activation_of_T_Cells_IL12, name = Activation of T Cells;IL12
	reaction_Activation_of_T_Cells_IL12=compartment_compartment*Function_for_Activation_of_T_cells__IL12(global_par_chi_A, x(5));

% Reaction: id = Activation_of_naive_T_cells_IL12, name = Activation of naive T cells;IL12
	reaction_Activation_of_naive_T_cells_IL12=compartment_compartment*Function_for_Activation_of_naive_T_cells_IL12(global_par_C_A, x(2));
...
% Reaction: id = natural_death_of_naive_T_cells, name = natural death of naive T cells
	reaction_natural_death_of_naive_T_cells=compartment_compartment*Function_for_natural_death_of_naive_T_cells(global_par_delta_A, x(5));
...
% Species:   id = Tumor_targeting_T_cells_T, name = Tumor-targeting T cells T, affected by kineticLaw
	xdot(4) = (1/(compartment_compartment))*(( 1.0 * reaction_Activation_of_T_Cells_IL12) + ( 1.0 * reaction_Activation_of_T_cells_4_1BBL) + ( 1.0 * reaction_Production_of_T_cells) + (-1.0 * reaction_natural_death_of_T_cells));
	
% Species:   id = Naive_T_cells_A, name = Naive T cells A, affected by kineticLaw
	xdot(5) = (1/(compartment_compartment))*(( 1.0 * reaction_Activation_of_naive_T_cells_IL12) + (-1.0 * reaction_natural_death_of_naive_T_cells));
...
function z=Function_for_Activation_of_T_cells__IL12(Chi_A,A), z=(Chi_A*A);end

function z=Function_for_Activation_of_T_cells__4_1BBL(C_T,I), z=(C_T*I);end

function z=Function_for_natural_death_of_naive_T_cells(delta_A,A), z=(delta_A*A);end

function z=Function_for_Activation_of_naive_T_cells_IL12(C_A,I), z=(C_A*I);end
