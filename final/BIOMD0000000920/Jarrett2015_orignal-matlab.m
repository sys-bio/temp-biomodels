...
x(4) = x(8);
	...
	xdot=zeros(8,1);
	% rateRule: variable = B
	xdot(5) = (global_par_g*(1-x(1)/global_par_K_B)+global_par_alpha_4*x(3)-global_par_beta_4*x(2))*x(1)+exp((-1)*global_par_gamma*time);
	% rateRule: variable = P
	xdot(6) = (global_par_alpha_1*x(3)+global_par_rho_1*x(1))*(1-x(2))-(global_par_beta_1*x(4)+global_par_mu_1*(1-x(1)/global_par_K_B))*x(2);
	% rateRule: variable = I
	xdot(7) = global_par_alpha_3*x(2)+global_par_rho_2*x(1)-(global_par_beta_3*x(4)+global_par_mu_3)*x(3);
	% rateRule: variable = A
	xdot(8) = global_par_alpha_2*x(2)-(global_par_beta_2*x(3)+global_par_mu_2*(1-x(1)/global_par_K_B))*x(4);
	
% Species:   id = B, name = B, involved in a rule 	xdot(1) = x(1);
	
% Species:   id = P, name = P, involved in a rule 	xdot(2) = x(2);
	
% Species:   id = I, name = I, involved in a rule 	xdot(3) = x(3);
	
% Species:   id = A, name = A, involved in a rule 	xdot(4) = x(4);
end
