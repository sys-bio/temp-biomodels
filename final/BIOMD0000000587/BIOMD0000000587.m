...
% Parameter:   id =  ModelValue_18, name = Initial for f
	global_par_ModelValue_18=3.63;
% Parameter:   id =  ModelValue_3, name = Initial for rho_b
	global_par_ModelValue_3=0.245950413223141;
% Parameter:   id =  ModelValue_2, name = Initial for rho_f
	global_par_ModelValue_2=0.8928;
...
% Reaction: id = G1____G0___R2, name = G1 -> G0 + R2
	reaction_G1____G0___R2=compartment_yeast*global_par_t_10*x(2);

% Reaction: id = G0___R2____G1, name = G0 + R2 -> G1
	reaction_G0___R2____G1=compartment_yeast*global_par_a_01*x(1)*x(8);

% Reaction: id = G0____G0___rR, name = G0 -> G0 + rR
	reaction_G0____G0___rR=compartment_yeast*global_par_rho_f*x(1);

% Reaction: id = G1____G1___rR, name = G1 -> G1 + rR
	reaction_G1____G1___rR=compartment_yeast*global_par_rho_b*x(2);

% Reaction: id = rR___, name = rR -> 
	reaction_rR___=compartment_yeast*global_par_delta_m*x(3);

% Reaction: id = rI___, name = rI -> 
	reaction_rI___=compartment_yeast*global_par_delta_m*x(5);

% Reaction: id = rR____rR___R, name = rR -> rR + R
	reaction_rR____rR___R=compartment_yeast*global_par_beta*x(3);
...
% Reaction: id = R___, name = R -> 
	reaction_R___=compartment_yeast*global_par_delta_p*x(6);

% Reaction: id = R___I____RI, name = R + I -> RI
	reaction_R___I____RI=compartment_yeast*global_par_gamma*x(6)*x(4);

% Reaction: id = _2___R____R2, name = 2 * R -> R2
	reaction__2___R____R2=compartment_yeast*global_par_gamma*x(6)^2;

% Reaction: id = R2____2___R, name = R2 -> 2 * R
	reaction_R2____2___R=compartment_yeast*global_par_epsilon_1*x(8);

% Reaction: id = RI____R___I, name = RI -> R + I
	reaction_RI____R___I=compartment_yeast*global_par_epsilon*x(7);

% Reaction: id = RI___, name = RI -> 
	reaction_RI___=compartment_yeast*global_par_delta_p*x(7);

% Reaction: id = R2___, name = R2 -> 
	reaction_R2___=compartment_yeast*global_par_delta_p*x(8);

% Reaction: id = G1___R2____G2, name = G1 + R2 -> G2
	reaction_G1___R2____G2=compartment_yeast*global_par_a_12*x(2)*x(8);

% Reaction: id = G2____G1___R2, name = G2 -> G1 + R2
	reaction_G2____G1___R2=compartment_yeast*global_par_t_21*x(9);

% Reaction: id = G2____G2___rR, name = G2 -> G2 + rR
	reaction_G2____G2___rR=compartment_yeast*global_par_rho_b*x(9);

% Reaction: id = G2___R2____G3, name = G2 + R2 -> G3
	reaction_G2___R2____G3=compartment_yeast*global_par_a_23*x(9)*x(8);

% Reaction: id = G3____G2___R2, name = G3 -> G2 + R2
	reaction_G3____G2___R2=compartment_yeast*global_par_t_32*x(10);

% Reaction: id = G3____G3___rR, name = G3 -> G3 + rR
	reaction_G3____G3___rR=compartment_yeast*global_par_rho_b*x(10);

	xdot=zeros(10,1);
	
% Species:   id = G0, name = G0, affected by kineticLaw
	xdot(1) = (1/(compartment_yeast))*(( 1.0 * reaction_G1____G0___R2) + (-1.0 * reaction_G0___R2____G1) + (-1.0 * reaction_G0____G0___rR) + ( 1.0 * reaction_G0____G0___rR));
	
% Species:   id = G1, name = G1, affected by kineticLaw
	xdot(2) = (1/(compartment_yeast))*((-1.0 * reaction_G1____G0___R2) + ( 1.0 * reaction_G0___R2____G1) + (-1.0 * reaction_G1____G1___rR) + ( 1.0 * reaction_G1____G1___rR) + (-1.0 * reaction_G1___R2____G2) + ( 1.0 * reaction_G2____G1___R2));
	
% Species:   id = rR, name = rR, affected by kineticLaw
	xdot(3) = (1/(compartment_yeast))*(( 1.0 * reaction_G0____G0___rR) + ( 1.0 * reaction_G1____G1___rR) + (-1.0 * reaction_rR___) + (-1.0 * reaction_rR____rR___R) + ( 1.0 * reaction_rR____rR___R) + ( 1.0 * reaction_G2____G2___rR) + ( 1.0 * reaction_G3____G3___rR));
	
% Species:   id = I, name = I, affected by kineticLaw
	xdot(4) = (1/(compartment_yeast))*((-1.0 * reaction_I___) + ( 1.0 * reaction_rI____rI___I) + (-1.0 * reaction_R___I____RI) + ( 1.0 * reaction_RI____R___I));
	
% Species:   id = rI, name = rI, affected by kineticLaw
	xdot(5) = (1/(compartment_yeast))*(( 1.0 * reaction_____rI) + (-1.0 * reaction_rI___) + (-1.0 * reaction_rI____rI___I) + ( 1.0 * reaction_rI____rI___I));
	
% Species:   id = R, name = R, affected by kineticLaw
	xdot(6) = (1/(compartment_yeast))*(( 1.0 * reaction_rR____rR___R) + (-1.0 * reaction_R___) + (-1.0 * reaction_R___I____RI) + (-2.0 * reaction__2___R____R2) + ( 2.0 * reaction_R2____2___R) + ( 1.0 * reaction_RI____R___I));
	
% Species:   id = RI, name = RI, affected by kineticLaw
	xdot(7) = (1/(compartment_yeast))*(( 1.0 * reaction_R___I____RI) + (-1.0 * reaction_RI____R___I) + (-1.0 * reaction_RI___));
	
% Species:   id = R2, name = R2, affected by kineticLaw
	xdot(8) = (1/(compartment_yeast))*(( 1.0 * reaction_G1____G0___R2) + (-1.0 * reaction_G0___R2____G1) + ( 1.0 * reaction__2___R____R2) + (-1.0 * reaction_R2____2___R) + (-1.0 * reaction_R2___) + (-1.0 * reaction_G1___R2____G2) + ( 1.0 * reaction_G2____G1___R2) + (-1.0 * reaction_G2___R2____G3) + ( 1.0 * reaction_G3____G2___R2));
	
% Species:   id = G2, name = G2, affected by kineticLaw
	xdot(9) = (1/(compartment_yeast))*(( 1.0 * reaction_G1___R2____G2) + (-1.0 * reaction_G2____G1___R2) + (-1.0 * reaction_G2____G2___rR) + ( 1.0 * reaction_G2____G2___rR) + (-1.0 * reaction_G2___R2____G3) + ( 1.0 * reaction_G3____G2___R2));
	
% Species:   id = G3, name = G3, affected by kineticLaw
	xdot(10) = (1/(compartment_yeast))*(( 1.0 * reaction_G2___R2____G3) + (-1.0 * reaction_G3____G2___R2) + (-1.0 * reaction_G3____G3___rR) + ( 1.0 * reaction_G3____G3___rR));
end
