...
%
% Model name = Panteleev2002_TFPImechanism_schmema2
%
% is http://identifiers.org/biomodels.db/MODEL1108260007
% is http://identifiers.org/biomodels.db/BIOMD0000000360
...
% Reaction: id = reaction_6, name = reaction_6	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_6_k1=10.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_6_k2=0.0;

	reaction_reaction_6=compartment_compartment*(reaction_reaction_6_k1*x(4)*x(6)-reaction_reaction_6_k2*x(9));

% Reaction: id = reaction_7, name = reaction_7	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_7_k1=0.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_7_k2=0.0;

	reaction_reaction_7=compartment_compartment*(reaction_reaction_7_k1*x(9)-reaction_reaction_7_k2*x(1)*x(7));

% Reaction: id = reaction_8, name = reaction_8	% Local Parameter:   id =  k1, name = k1
	reaction_reaction_8_k1=0.0;
	% Local Parameter:   id =  k2, name = k2
	reaction_reaction_8_k2=0.0;

	reaction_reaction_8=compartment_compartment*(reaction_reaction_8_k1*x(9)-reaction_reaction_8_k2*x(8));

	xdot=zeros(9,1);
	
% Species:   id = VIIa_TF, name = VIIa_TF, affected by kineticLaw
	xdot(1) = (1/(compartment_compartment))*((-1.0 * reaction_reaction_1) + ( 1.0 * reaction_reaction_3) + (-1.0 * reaction_reaction_5) + ( 1.0 * reaction_reaction_7));
...
% Species:   id = Xa_TFPI, name = Xa_TFPI, affected by kineticLaw
	xdot(7) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_4) + (-1.0 * reaction_reaction_5) + ( 1.0 * reaction_reaction_7));
	
% Species:   id = Xa_TFPI_VIIa_TF, name = Xa_TFPI_VIIa_TF, affected by kineticLaw
	xdot(8) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_5) + ( 1.0 * reaction_reaction_8));
	
% Species:   id = VIIa_TF_Xa_TFPI, name = VIIa_TF_Xa_TFPI, affected by kineticLaw
	xdot(9) = (1/(compartment_compartment))*(( 1.0 * reaction_reaction_6) + (-1.0 * reaction_reaction_7) + (-1.0 * reaction_reaction_8));
end
