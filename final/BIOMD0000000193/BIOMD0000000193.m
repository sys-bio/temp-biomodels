...
%
% Model name = Ibrahim2008_MCC_assembly_model_KDM
%
% is http://identifiers.org/biomodels.db/MODEL1667758030
% is http://identifiers.org/biomodels.db/BIOMD0000000193
% isDescribedBy http://identifiers.org/pubmed/18675311
%
...
% Reaction: id = R2
	reaction_R2=compartment_Cytoplasm*global_par_u*global_par_k2f*x(3)*x(4);

% Reaction: id = R3
	reaction_R3=compartment_Cytoplasm*global_par_k3f*x(5);
...
% Reaction: id = R6
	reaction_R6=compartment_Cytoplasm*global_par_kf6*x(2)*x(4);
...
% Species:   id = OMad2, name = O-Mad2, affected by kineticLaw
	xdot(2) = (1/(compartment_Cytoplasm))*((-1.0 * reaction_R1) + ( 1.0 * reaction_R3) + (-1.0 * reaction_R6));
...
% Species:   id = Cdc20, name = Cdc20, affected by kineticLaw
	xdot(4) = (1/(compartment_Cytoplasm))*((-1.0 * reaction_R2) + ( 1.0 * reaction_R3) + (-1.0 * reaction_R5) + (-1.0 * reaction_R6));
...
% Species:   id = Bub3_BubR1, name = Bub3:BubR1, affected by kineticLaw
	xdot(6) = (1/(compartment_Cytoplasm))*((-1.0 * reaction_R4) + (-1.0 * reaction_R5));
	
% Species:   id = MCC, name = MCC, affected by kineticLaw
	xdot(7) = (1/(compartment_Cytoplasm))*(( 1.0 * reaction_R4));
