...
%
% Model name = Tyson2003_Substrate_Depletion_Osc
%
% is http://identifiers.org/biomodels.db/MODEL1102100006
% is http://identifiers.org/biomodels.db/BIOMD0000000307
...
% Compartment: id = env, name = env, constant
	compartment_env=1.0;
% Parameter:   id =  k0_prime, name = k0_prime
	global_par_k0_prime=0.01;
...
% assignmentRule: variable = Ep
	x(4)=goldbeter_koshland(global_par_k3*x(1), global_par_k4, global_par_J3, global_par_J4)*global_par_Et;
% assignmentRule: variable = E
	x(5)=global_par_Et-x(4);

% Reaction: id = r0
	reaction_r0=compartment_env*(global_par_k0_prime+global_par_k0*x(4))*x(2);
...
% Reaction: id = r3
	reaction_r3=compartment_env*global_par_k3*x(1)*x(5)/(global_par_Km3+x(5));

% Reaction: id = r4
	reaction_r4=compartment_env*global_par_k4*x(4)/(global_par_Km4+x(4));
...
% Species:   id = X, name = X, affected by kineticLaw
	xdot(2) = (1/(compartment_env))*((-1.0 * reaction_r0) + ( 1.0 * reaction_r1));
	
% Species:   id = S, name = S
%WARNING speciesID: S, constant= false  , boundaryCondition = true but is not involved in assignmentRule, rateRule or events !
	xdot(3) = 0.0;
	
% Species:   id = Ep, name = Ep, involved in a rule 	xdot(4) = x(4);
	
% Species:   id = E, name = E, involved in a rule 	xdot(5) = x(5);
end
